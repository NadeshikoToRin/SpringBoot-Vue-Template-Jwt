...
/**
 * JwtUtils 类提供了生成、解析和验证JWT（JSON Web Token）的方法，主要用于用户身份验证和授权管理。
 */
@Component
public class JwtUtils {
    ...
    /**
     * 使指定的JWT token无效，即将其加入黑名单。
     *
     * @param headerToken 包含"Bearer "前缀的JWT token字符串
     * @return 如果成功使token无效，返回true；否则返回false
     */
    ...
    /**
     * 将指定的token加入黑名单，设置其过期时间。
     *
     * @param uuid JWT的唯一标识符
     * @param expiresAt JWT的过期时间
     * @return 如果token成功加入黑名单，返回true；否则返回false
     */
     ...
     *
     * @param uuid JWT的唯一标识符
     * @return 如果token在黑名单中，返回true；否则返回false
     */
    ...
    public String createJwt(UserDetails userDetails, int id, String username) {
        Algorithm algorithm = Algorithm.HMAC256(key); // 使用HMAC256算法创建算法实例
        Date expire = this.expireTime(); // 获取token的过期时间
    ...
    public Date expireTime() {
        ...
        calendar.add(Calendar.HOUR, expire * 24); // 在当前时间上加上配置的过期时间（以小时为单位，乘以24）
    ...
    private String convertToken(String hearToken) {
        if (hearToken == null || !hearToken.startsWith("Bearer ")) { // 检查token是否为空且以"Bearer "开头
            return null;
        }
        return hearToken.substring(7); // 返回去掉"Bearer "前缀的token字符串
    }
